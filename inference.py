...
from ensemble_inference import nmw_ensemble
...
class DrawBBox:
    ...
    def draw_bbox(self, images, boxes, scores, labels, save_path='infer_image'):

        Path(save_path).mkdir(exist_ok=True)

        for image_name, box, score, label in zip(images, boxes, scores, labels):
            ...
            self.draw_bboxes_from_output(image, boxes=box, scores=score, labels=label, ax=axes)
...
def main(imgs_path: list or str = None, json_path: str = None):
    ...
    work_dir = '~/Git/p_stage_obj_det/mmdet/config/work_dirs/test'
    config_file = '~/Git/object-detection-level2-cv-10/mmdet_config/models/swin/swin-s_fpn_htc_soft-nms_AdamW-2x.py'
    checkpoint_file = '4th-model_epoch_4.pth'

    df = infer_module(work_dir=work_dir, config_file=config_file, checkpoint_file=checkpoint_file)
    dfs.append(df)
